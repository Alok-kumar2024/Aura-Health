...
class AnalysisResultScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final hasNegative = result.interactions.any((i) => i.relation == "NEGATIVE_INTERACTION");

    // 2. Theme Colors
    final Color statusColor = hasNegative ? const Color(0xFFD32F2F) : const Color(0xFF2E7D32); // Red vs Green
    final Color bgTint = statusColor.withOpacity(0.05);
    final String statusTitle = hasNegative ? "Interaction Detected" : "Safe to Consume";
    final IconData statusIcon = hasNegative ? Icons.warning_amber_rounded : Icons.check_circle_rounded;
        ...
        child: Column(
          ...
          children: [
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      _buildTag(result.drug, Colors.blue),
                      Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 8.0),
                        child: Icon(Icons.add, size: 16, color: Colors.grey.shade400),
                      ),
                      _buildTag(result.food, Colors.orange),
                    ],
                  ),
                ],
              ),
            ...
            if (result.interactions.isEmpty)
              ...
              ...result.interactions.map((i) => _buildInteractionCard(i, statusColor)),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.science, size: 20, color: Colors.blueGrey.shade700),
              ...
              Text(
                item.reason,
                style: TextStyle(fontSize: 14, color: Colors.blueGrey.shade700, height: 1.4),
              ),
            ],
          ),
        ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.lightbulb_outline_rounded, size: 20, color: Colors.green.shade700),
                  const SizedBox(width: 8),
                  Text(
                    "Try this instead",
              ...
              Text(
                item.alternatives,
                style: TextStyle(fontSize: 15, color: Colors.green.shade900, fontWeight: FontWeight.w500),
              ),
            ],
          ),
        ),
  ...
  Widget _buildSafeState() {
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.thumb_up_alt_rounded, size: 40, color: Colors.green.shade300),
          const SizedBox(height: 16),
          const Text(
